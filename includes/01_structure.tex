...
premenných v~úvode hlavného súboru \verb|thesis.tex| v makre \verb|\FEIsetup|.

Šablóna sa postará o automatické vygenerovanie obálky a titulnej strany podľa
poskytnutých informácií.
Pokročilí študenti môžu toto automatické generovanie vypnúť nastavením
parametra \verb|automaticfront| na \verb|false| balíka \verb|FEIstylethesis|.
Potom je možné vygenerovanú obálku a titulnú stranu z AIS-u vložiť priamo
pomocou príkazu \verb|\includepdf| z~balíčka \verb|pdfpages|.
V prípade vypnutia automatického generovania preambuly práce je potrebné vložiť
zadanie, poďakovanie, abstrakty a obsah \emph{manuálne}.

Zadanie odporúčame vložiť vyplnením cesty v makre
\verb|\FEIsetup| tak, že najprv uložíme PDF súbor
so zadaním do priečinka \verb|includes| a prepíšeme
názov súboru v~kľúči \verb|assignment| makra \verb|\FEIsetup|.

\subsection{Poďakovanie}
\textbf{Nepovinná}, ale veľmi obľúbená časť práce.
...
kľúč \verb|assignment| v príkaze \verb|\FEIsetup|.
...
v~súboroch \verb|appendix.tex| a~\verb|appendixEN.tex| v~priečinku \verb|includes|.
Do dokumentu ich vloží makro šablóny
\verb|\FEIsetup| v~hlavnom súbore \verb|thesis.tex|.
Makro obsahuje cesty k textom slovenského a~anglického abstraktu.
Pod abstrakty sa zároveň automaticky vytlačia
zoznamy kľúčových slov v príslušnom jazyku.
...
s~príponou \verb|.toc| a~na základe neho generuje finálnu podobu obsahu.
Z~toho dôvodu je potrebné kompilátor \LaTeX-u
spustiť minimálne dvakrát za sebou.

V tejto šablóne je obsah automaticky generovaný po abstraktoch. Vytvorenie
obsahu má na starosti makro \verb|\FEItoc|, ktoré musíte zavolať, ak ste vypli
automatické generovanie preambuly.

\subsection*{\normalsize Zoznam ilustrácií,
obrázkov a tabuliek}
Sú to nepovinné prehľady tzv. plávajúcich objektov.
\LaTeX\ pozná na tento účel dva príkazy: \verb|\listoffigures| a~\verb|\listoftables|.
Zvlášť užitočný príkaz zo šablóny \verb|FEIstylethesis| je \verb|\FEIlistOfFiguresAndTables|.
Vytvorí totiž spojený zoznam ob\-ráz\-kov a~tabuliek s jedným nadpisom.
...
V textových výstupoch vedecko-technických odborov sa používac
...
Makro \verb|\printacronyms| vytvorí abecedne zoradený zoznam.

Ak sa rozhodneme pre túto možnosť,
treba v hlavnom súbore \verb|thesis.tex| odstrániť
znak komentára pred príkazmi
\verb|\makeglossaries|, \verb|\loadglsentries{includes/| \verb|glossary}|
a~\verb|\printacronyms|. Pozor, tieto tri
...
Obsah tohto súboru je možné vysádzať do práce štan\-dard\-ným makrom
\verb|\include| pre vloženie obsahu súboru, ktorého parameter je cesta
k~spomínanému súboru.
...
Tieto typy zoznamov vytvoria makrá \verb|\listofalgorithms|, \verb|\lstlistoflistings| a~sú špecifické pre informatické odbory.
...
Do hlavného dokumentu \verb|thesis.tex| ho načíta makro \verb|\input{includes/core}|.
...
Každý dodatok je potom potrebné načítať v~hlavnom súbore \verb|thesis.tex|
pomocou makra \verb|\include|, napr. \verb|\include{includes/attachmentA}|.
Načítanie dodatkov musí byť umiestnené \textbf{za} makrom \verb|\appendix|,
ináč nebudú správne sádzané nadpisy kapitol dodatkov.
